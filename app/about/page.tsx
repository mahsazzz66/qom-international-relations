...
export default function AboutPage() {
  ...
  return (
    <div>
      <PageHero
        page="about"
        title={t("About the International Relations & Communications Department")}
        description={t("The official channel between Qom Municipality and the international community.")}
        icon={
      ...
      <div className="mx-auto max-w-[1280px] px-6 pt-18 pb-24">
        ...
        <div id="about-objectives-detail" className="mb-18">
          ...
          <div className="grid gap-px bg-navy/[.12] dark:bg-dark-fill" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))" }}>
            {OBJECTIVES.map((o, i) => (
              <div key={o.title} className="bg-white dark:bg-dark-surface-2 p-7.5">
                <div className="font-mono mb-4.5 text-xs text-gold">{String(i + 1).padStart(2, "0")}</div>
                <h3 className="m-0 mb-2.5 font-serif text-lg font-medium">{t(o.title)}</h3>
                <p className="m-0 text-sm leading-[1.65] text-slate dark:text-dark-ink-dim">{t(o.body)}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div id="about-structure" className="mb-18">
          ...
          <div className="grid gap-6" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))" }}>
            {STRUCTURE.map((s) => (
              <div key={s.title} className="border border-t-3 border-navy/[.12] dark:border-dark-line border-t-gold bg-white dark:bg-dark-surface-2 p-6.5">
        ...
        <div id="about-strategy" className="bg-navy dark:bg-dark-navy p-9 text-bg sm:p-16">
          ...
          <div className="grid gap-8.5" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(230px, 1fr))" }}>
            {PILLARS.map((p) => (
              <div key={p.title}>
                <div className="font-mono mb-3 text-[11px] tracking-[.18em] text-gold uppercase">{t(p.n)}</div>
                <h3 className="m-0 mb-2.5 font-serif text-[19px] font-medium">{t(p.title)}</h3>
                <p className="m-0 text-sm leading-[1.7] text-[rgba(250,248,244,.75)]">{t(p.body)}</p>
              </div>
            ))}
          </div>
        </div>
